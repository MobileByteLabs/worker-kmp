---
module: cmp-worker-compose-all
artifact: io.github.mobilebytelabs:cmp-worker-compose-all
version: UNKNOWN
package: com.mobilebytelabs.kmptoolkit.worker.compose.all
...
# cmp-worker-compose-all — Development

> Single source of truth for development state of `cmp-worker-compose-all` (KMP library module). Per RULE-LIB-DEVELOPMENT-MD-001.
...
|----------|---------|-----------------|-------|-------|----------|
| `io.github.mobilebytelabs:cmp-worker-compose-all` | `com.mobilebytelabs.kmptoolkit.worker.compose.all` | `UNKNOWN` | [Central](https://central.sonatype.com/artifact/io.github.mobilebytelabs/cmp-worker-compose-all) | 2026-05-30 | experimental |

**Module purpose (one paragraph):** <!-- AUTHOR: WIP — initial draft from 2026-05-30. One-paragraph module purpose (≤200 words). Seed from idea-layer/cmp-worker-compose-all/SPEC.md if present. -->
...
### cmp-worker-compose-all.klib.api
```
...
|------|--------|----|---------|-------|
| (no open PRs labeled `cmp-worker-compose-all` — refresh via `gh pr list --label cmp-worker-compose-all` then re-run scan) | — | — | — | — |
...
|-------------|--------|---------|
| T0 (Crashlytics attribution) | enabled | custom_key: `library:cmp-worker-compose-all@UNKNOWN` (set on init by FirebaseCrashlyticsAttributionHook) |
...
tiers:
  ...
  T1: enabled
  T2: opted-out
custom_key_format: "library:cmp-worker-compose-all@UNKNOWN"
event_schema: []  # populate when T2 enabled — see library-runtime-observability epic AC #12-13
consumer_opt_in: "lib-integrate.properties#cmp-worker-compose-all.observability_opt_in"
```

**Consumer opt-in:** controlled via `cmp-worker-compose-all.observability_opt_in=true` in consumer's `lib-integrate.properties`.
