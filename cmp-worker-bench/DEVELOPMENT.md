---
module: cmp-worker-bench
artifact: io.github.mobilebytelabs:cmp-worker-bench
version: UNKNOWN
package: com.mobilebytelabs.kmptoolkit.worker.bench
...
# cmp-worker-bench — Development

> Single source of truth for development state of `cmp-worker-bench` (KMP library module). Per RULE-LIB-DEVELOPMENT-MD-001.
...
|----------|---------|-----------------|-------|-------|----------|
| `io.github.mobilebytelabs:cmp-worker-bench` | `com.mobilebytelabs.kmptoolkit.worker.bench` | `UNKNOWN` | [Central](https://central.sonatype.com/artifact/io.github.mobilebytelabs/cmp-worker-bench) | 2026-05-30 | experimental |

**Module purpose (one paragraph):** <!-- AUTHOR: WIP — initial draft from 2026-05-30. One-paragraph module purpose (≤200 words). Seed from idea-layer/cmp-worker-bench/SPEC.md if present. -->
...
|------|--------|----|---------|-------|
| (no open PRs labeled `cmp-worker-bench` — refresh via `gh pr list --label cmp-worker-bench` then re-run scan) | — | — | — | — |
...
|-------------|--------|---------|
| T0 (Crashlytics attribution) | enabled | custom_key: `library:cmp-worker-bench@UNKNOWN` (set on init by FirebaseCrashlyticsAttributionHook) |
...
tiers:
  ...
  T1: enabled
  T2: opted-out
custom_key_format: "library:cmp-worker-bench@UNKNOWN"
event_schema: []  # populate when T2 enabled — see library-runtime-observability epic AC #12-13
consumer_opt_in: "lib-integrate.properties#cmp-worker-bench.observability_opt_in"
```

**Consumer opt-in:** controlled via `cmp-worker-bench.observability_opt_in=true` in consumer's `lib-integrate.properties`.
