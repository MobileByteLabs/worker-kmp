...
# are added without a corresponding SECURITY.md row update in the same PR.
#
# Usage:
...
#
# Exit codes:
#   0  OK — either no surface change, or SECURITY.md was also touched in the diff range
#   1  Surface added without SECURITY.md update — CI should fail with a clear pointer
...
# Directories that introduce attack surfaces (per Phase 10 SECURITY.md scope)
SURFACE_DIRS=(
...
# Surface was touched — SECURITY.md must also be in the diff
if echo "${CHANGED_FILES}" | grep -q "^SECURITY\.md$"; then
    echo "✓ Attack surface touched AND SECURITY.md updated in same PR"
    exit 0
fi

echo ""
echo "✗ Attack surface touched but SECURITY.md NOT updated."
echo ""
...
echo "  corresponding update to SECURITY.md's STRIDE matrix (new row OR row update)."
echo ""
echo "  Either:"
echo "    1. Add/update a SECURITY.md row reflecting the new threat profile, OR"
