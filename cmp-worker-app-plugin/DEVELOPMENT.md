---
module: cmp-worker-app-plugin
artifact: io.github.mobilebytelabs:cmp-worker-app-plugin
version: UNKNOWN
package: com.mobilebytelabs.kmptoolkit.worker.app.plugin
...
# cmp-worker-app-plugin — Development

> Single source of truth for development state of `cmp-worker-app-plugin` (KMP library module). Per RULE-LIB-DEVELOPMENT-MD-001.
...
|----------|---------|-----------------|-------|-------|----------|
| `io.github.mobilebytelabs:cmp-worker-app-plugin` | `com.mobilebytelabs.kmptoolkit.worker.app.plugin` | `UNKNOWN` | [Central](https://central.sonatype.com/artifact/io.github.mobilebytelabs/cmp-worker-app-plugin) | 2026-05-30 | experimental |

**Module purpose (one paragraph):** <!-- AUTHOR: WIP — initial draft from 2026-05-30. One-paragraph module purpose (≤200 words). Seed from idea-layer/cmp-worker-app-plugin/SPEC.md if present. -->
...
|------|--------|----|---------|-------|
| (no open PRs labeled `cmp-worker-app-plugin` — refresh via `gh pr list --label cmp-worker-app-plugin` then re-run scan) | — | — | — | — |
...
|-------------|--------|---------|
| T0 (Crashlytics attribution) | enabled | custom_key: `library:cmp-worker-app-plugin@UNKNOWN` (set on init by FirebaseCrashlyticsAttributionHook) |
...
tiers:
  ...
  T1: enabled
  T2: opted-out
custom_key_format: "library:cmp-worker-app-plugin@UNKNOWN"
event_schema: []  # populate when T2 enabled — see library-runtime-observability epic AC #12-13
consumer_opt_in: "lib-integrate.properties#cmp-worker-app-plugin.observability_opt_in"
```

**Consumer opt-in:** controlled via `cmp-worker-app-plugin.observability_opt_in=true` in consumer's `lib-integrate.properties`.
