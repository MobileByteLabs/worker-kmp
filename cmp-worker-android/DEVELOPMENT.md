---
module: cmp-worker-android
artifact: io.github.mobilebytelabs:cmp-worker-android
version: UNKNOWN
package: com.mobilebytelabs.kmptoolkit.worker.android
...
# cmp-worker-android — Development

> Single source of truth for development state of `cmp-worker-android` (KMP library module). Per RULE-LIB-DEVELOPMENT-MD-001.
...
|----------|---------|-----------------|-------|-------|----------|
| `io.github.mobilebytelabs:cmp-worker-android` | `com.mobilebytelabs.kmptoolkit.worker.android` | `UNKNOWN` | [Central](https://central.sonatype.com/artifact/io.github.mobilebytelabs/cmp-worker-android) | 2026-05-30 | experimental |

**Module purpose (one paragraph):** <!-- AUTHOR: WIP — initial draft from 2026-05-30. One-paragraph module purpose (≤200 words). Seed from idea-layer/cmp-worker-android/SPEC.md if present. -->
...
|--------|:------------------:|:---------:|:------------------------:|:---------:|---------------|-------|
| androidMain | ✅ | ✅ real | 0 | 10 | 2026-05-30 | — |
...
|------|--------|----|---------|-------|
| (no open PRs labeled `cmp-worker-android` — refresh via `gh pr list --label cmp-worker-android` then re-run scan) | — | — | — | — |
...
|-------------|--------|---------|
| T0 (Crashlytics attribution) | enabled | custom_key: `library:cmp-worker-android@UNKNOWN` (set on init by FirebaseCrashlyticsAttributionHook) |
...
tiers:
  ...
  T1: enabled
  T2: opted-out
custom_key_format: "library:cmp-worker-android@UNKNOWN"
event_schema: []  # populate when T2 enabled — see library-runtime-observability epic AC #12-13
consumer_opt_in: "lib-integrate.properties#cmp-worker-android.observability_opt_in"
```

**Consumer opt-in:** controlled via `cmp-worker-android.observability_opt_in=true` in consumer's `lib-integrate.properties`.
