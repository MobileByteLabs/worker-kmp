...
internal fun Set<String>.extractKmpId(): Uuid? =
    firstOrNull { it.startsWith(TAG_KMP_ID_PREFIX) }
        ?.removePrefix(TAG_KMP_ID_PREFIX)
        ?.let { runCatching { Uuid.parse(it) }.getOrNull() }

internal fun Set<String>.userTags(): Set<String> =
    filterNot { it.startsWith(TAG_KMP_ID_PREFIX) }.toSet()
...
internal fun ExistingPeriodicWorkPolicy.toAndroid(): AndroidExistingPeriodicWorkPolicy =
    when (this) {
    ...
    keyValueMap().forEach { (key, value) ->
        when (value) {
            ...
            is Array<*> -> {
                ...
                when {
                    value.isArrayOf<String>() -> builder.putStringArray(key, (value as Array<String>).map { it }.toTypedArray())
